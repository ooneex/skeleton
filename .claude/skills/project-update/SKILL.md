...
- **Config with merge points — `package.json` (root), `tsconfig.json`, `biome.jsonc`, `.zed/settings.json`, `renovate.json`, `.env.yml`.** Structural merge: adopt new/changed template keys, **keep every local key**. Root `package.json`: refresh `scripts`/`workspaces` from the template (add new, keep local extras) but **never drop or downgrade a local dependency**. `.env.yml`: add new keys with template defaults, **keep every existing local value** — never reset a URL, secret, or tuned setting.
