...
- **Run independent units concurrently.** After step 2 groups the batch into stacks, each stack and each standalone issue is an independent unit — steps 3–4 for one unit never touch another unit's worktree or branch. When the batch has more than one unit, dispatch each unit's steps 3–4 to its own general-purpose sub-agent via the Agent tool, issuing every independent unit's Agent call together in one message so they run in parallel; give each sub-agent its unit's `(module, ID)` pairs and let it create and own that unit's worktree end-to-end (setup, branch, fixer dispatch, commit, push, PR, link-back, `issue:check`, teardown) per steps 3–4 below. Only issues **within the same stack** stay sequential — they share one worktree and one checked-out branch chain (step 3), so a single sub-agent runs that stack's layers one after another. A batch of exactly one unit runs inline, no sub-agent wrapper needed.
...
**Within a unit, dispatch sequentially, one issue at a time, in dependency order** — issues in the same stack share one worktree and one checked-out branch, so concurrent runs would still clobber each other even though the worktree isolates them from the user's own tree. Let each finish before switching to the next issue's branch. **Across units** (see "Run independent units concurrently" above), this dispatch-and-finalise sequence runs in parallel — each unit's sub-agent works only inside its own worktree, so nothing collides. If a dispatched issue has a dependency **not** in the batch and not yet `In Review`/`Done`, the fixer stops and reports it — carry that into the summary.
