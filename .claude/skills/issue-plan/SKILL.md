...
- Write runnable steps in implementation order — command / route / input — each with its **expected result**. Prefer project tooling (`talos check --strict --logs`, a specific `curl`/route, `talos e2e:run`, `talos app:start`).
- Cover every `dod` item, including meaningful edge/error cases.
- Match the module type: **backend** (module/api/microservice) tests endpoints/services/migrations with unit/integration tests (`bun:test`) — never Playwright e2e for backend logic; **SPA/design/storybook** tests rendered routes and interactions (`talos app:start` + browser flow, `talos e2e:run` when a spec exists).
