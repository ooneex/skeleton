...
5. **Check** — `talos check --strict` (lint, format, test). Fix every failure. Verify mentally against the model: each `meta.title` yields a sidebar entry under its `group`; dotted titles nest; the `size` control resizes the composed preview; every option shows its `usage`.
