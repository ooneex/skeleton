...
Before self-review, run `/ui-verify` on every route and state changed by the issue, even when the issue does not list a browser-flow testing step. Exercise the primary flow and changed interactions with trusted Bun.WebView input at desktop and mobile viewports, inspect screenshots, and fix rendered regressions. Keep `/e2e-create` for durable regression coverage; `/ui-verify` is the mandatory live completion proof.
