...
- **Improvement / Code Quality / Database** — dead/duplicated code, weak typing (`any`), non-arrow functions where the `optimize` skill requires them, type/interface naming violations, missing exception classes, magic values, missing indexes, unsafe or non-idempotent migrations (a `CREATE` in `up()` without `IF NOT EXISTS`), migrations that never run (bare `<version>.ts` filename, missing from the `migrations.ts` barrel, or barrel not imported by `bin/migration/{up,down}.ts`).
