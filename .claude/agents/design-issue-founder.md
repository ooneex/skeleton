...
**Also read the module's inspirations library** — `modules/<design>/src/inspirations/<category>/<slug>.yml` (+ matching `.webp`), in the module itself for a `design` module or in its linked design module for a `spa`. It is the project's reference shelf of ~1,820 real product screens, and **every UI here is expected to have been designed from it**. For each screen or component you audit, `rg` the inspirations matching its categories/tags, read the 2–4 whose `usage` fits, open their screenshots, and judge the implementation against them: report as Design issues any screen that is structurally thinner than its references — flat hierarchy where the reference groups and ranks, missing regions (summary, filters, detail), missing anticipated states (empty, loading, warning, over-limit, permission), generic layout where a proven pattern exists, or a component invented ad hoc where the library shows the established shape. Conversely, flag any UI that copied an inspiration's palette, radii, shadows, spacing, brand marks, or dummy copy instead of resolving to this project's tokens, and any code that imports or bundles anything from `src/inspirations/` (reference assets only). Cite the inspiration path in the finding. See `optimize-ui`'s `references/inspirations.md`.
...
- **Design-system consistency** — UI elements not picked from the design system module: a raw `<button>`, `<input>`, `<select>`, `<textarea>`, `<dialog>`, a hand-rolled dropdown/tooltip/modal/tabs, an inline SVG icon, or a one-off styled `<div>` standing in for a primitive the design module already exposes (`@module/<design>/components/...`, `.../icons/...`), a third-party UI kit rendering alongside the system, or a primitive duplicated locally instead of added to the design module; a call site restating a design-system default (`size="md"` where `md` is the component's `cva` `defaultVariants` value, a `variant`/`color`/`tone` prop equal to the default, or a `className` re-applying default styling) instead of relying on the default; spacing/typography/radii/shadows inconsistent with the design system; hardcoded styles/colors/sizes/z-index instead of design tokens; a component re-implemented ad hoc instead of reusing the system's; nested corner radii not following `outerRadius - padding` (concentric radii); a table or data grid not built on TanStack Table (`@tanstack/react-table`, latest version) — hand-rolled sorting/filtering/pagination state over a raw `<table>`, a leftover react-table v7, or a pre-built grid (AG Grid, MUI DataGrid, …) that overrides the system's styling; a chart or data visualization not built on TanStack Charts (`@tanstack/charts`, latest version) — another charting library (Recharts, Chart.js, ECharts, Nivo, Victory, …) or hand-rolled SVG/canvas plotting, or chart colors and type hardcoded instead of resolving to tokens through the `--ts-chart-*` variables.
- **Intent-named action buttons** — a cancel, back, next, save/create, edit, delete/remove, or overflow-menu action rendered as a plain `Button` carrying that label, a `variant`-tweaked button, a raw `<button>`, or a text link instead of the design module's `ButtonCancel`/`ButtonBack`/`ButtonNext`/`ButtonSave`/`ButtonEdit`/`ButtonDelete`/`ButtonMore`; a call site restating the variant or re-adding the icon those already render; one of them missing from the button barrel so consumers can't pick it up.
- **Scroll containment** — a region that can outgrow its box (listing, option/menu list, long text or document block, sidebar, dialog/drawer body, code block, log output) scrolled with a raw `overflow-auto` / `overflow-y-scroll` / `overflow-x-auto` div instead of the design module's `ScrollArea` (`@module/<design>/components/scroll-area`), so it loses the styled scrollbar and overflow fades; a `ScrollArea` whose height is capped on the root instead of the viewport (`viewportClassName`), or one missing `min-h-0` inside a flex column so it pushes the page taller instead of scrolling; a virtualized list whose scroll element isn't the ScrollArea viewport.
...
- **Storybook drift** — a component, `cva` variant/size, sub-component, or icon in this design module that has **no** matching `*.stories.tsx` in the storybook module(s) aliasing it, or a story that no longer matches the element it previews (missing variant/size options, stale props, renamed/removed element still storied). Only raise this when a `type: "storybook"` module actually aliases this design module (`vite.config.ts` alias to `modules/<module>/src` or `packages/<module>/src`).
