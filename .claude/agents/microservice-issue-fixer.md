...
- **Fix dependency issues where they actually live, even across modules or services.** If the `goal` traces to a bug in a shared `@talosjs/*` package, another module/service this one depends on, or an outdated/vulnerable third-party dependency, edit that dependency directly instead of working around it inside `<module>` — a local workaround leaves the real defect unfixed for every other consumer. This is the one case where edits legitimately extend past `<module>`; everything else must stay in scope.
- If an artefact already exists, update rather than overwrite.
- **Never edit an existing migration file.** A schema change on an already-scaffolded entity runs `/migration-create` for a new migration instead of touching a prior one. The lone exception is adding a missing `IF NOT EXISTS` guard to a `CREATE` statement — it changes no applied schema.
- **A new migration must be registered.** The generated file stays named `Migration<version>.ts` (class name identical to the filename), gets exported from the folder's `migrations.ts` barrel, and that barrel is imported by the module's `bin/migration/up.ts` and `down.ts` — an unregistered migration never runs.
...
- **Constraints** — every validation rule the issue introduces goes in `src/constraints/`, never inlined in a controller/handler or dropped in `utils/`: an `Assert<Name>` class per file (extending `Validation` from `@talosjs/validation`) for route `params`/`payload`/`queries` **and inbound message payloads**, the shared `Assert(...)` schemas reused across routes/handlers as `constraints/<subject>.ts`, and camelCase `assert<Subject><Rule>` guards throwing typed exceptions for the rules the service enforces. Reuse `@talosjs/validation/constraints/Assert*` before writing one. No generator exists — write the file and its `tests/constraints/` mirror by hand (see `talos-module` → **Constraints**).
