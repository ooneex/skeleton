...
- **Constraints** — every validation rule the issue introduces goes in `src/constraints/`, never inlined in the controller or dropped in `utils/`: an `Assert<Name>` class per file (extending `Validation` from `@talosjs/validation`) for route `params`/`payload`/`queries`, the shared `Assert(...)` schemas several routes reuse as `constraints/<subject>.ts`, and camelCase `assert<Subject><Rule>` guards throwing typed exceptions for the rules the service enforces. Reuse `@talosjs/validation/constraints/Assert*` (`AssertId`, `AssertEmail`, `AssertName`, `AssertCountryCode`, `AssertLocale`, `AssertFile`, …) before writing one. No generator exists — write the file and its `tests/constraints/` mirror by hand (see `talos-module` → **Constraints**).
