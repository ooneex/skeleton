...
- Never pad a coverage rate: a line executed by a test that asserts nothing is uncovered in every sense that matters. Re-run `talos coverage:check --strict --modules=<module> --logs` after pruning so the report reflects the tests that are actually left.
