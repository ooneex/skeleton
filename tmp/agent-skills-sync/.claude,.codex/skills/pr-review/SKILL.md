...
If `talos project:check --strict --logs` failed or any `dod` item is unmet or mis-checked, leave the state as `In Review` — do not promote an issue with blockers. For frontend modules, an e2e failure, a `testing` step with no covering spec, or missing/failed `/ui-verify` proof for a rendered change is also a blocker.
