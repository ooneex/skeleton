use dioxus::prelude::*;

#[derive(Props, Clone, PartialEq)]
pub struct PolaroidPhotoIconProps {
    #[props(extends = svg, extends = GlobalAttributes)]
    pub attributes: Vec<Attribute>,
}

#[component]
pub fn PolaroidPhotoIcon(props: PolaroidPhotoIconProps) -> Element {
    rsx! {
        svg {
            height: "16",
            width: "16",
            view_box: "0 0 24 24",
            xmlns: "http://www.w3.org/2000/svg",
            fill: "none",
            ..props.attributes,
            path {
                fill_rule: "evenodd",
                clip_rule: "evenodd",
                d: "M3 17H21V21H3V17Z",
                fill: "currentColor",
            }
            path {
                d: "M16.4529 7.47223L19 10.2395L19 16H9.5L16.4529 7.47223Z",
                fill: "currentColor",
                "data-color": "color-2",
            }
            path {
                fill_rule: "evenodd",
                clip_rule: "evenodd",
                d: "M19 2C20.6569 2 22 3.34315 22 5L22 19C22 20.6569 20.6569 22 19 22L5 22C3.34314 22 2 20.6569 2 19L2 5C2 3.34314 3.34315 2 5 2L19 2ZM20 5C20 4.44772 19.5523 4 19 4L5 4C4.44772 4 4 4.44771 4 5L4 19C4 19.5523 4.44771 20 5 20L19 20C19.5523 20 20 19.5523 20 19L20 5Z",
                fill: "currentColor",
            }
            path {
                d: "M9 11C10.1046 11 11 10.1046 11 9C11 7.89543 10.1046 7 9 7C7.89543 7 7 7.89543 7 9C7 10.1046 7.89543 11 9 11Z",
                fill: "currentColor",
                "data-color": "color-2",
            }
        }
    }
}
