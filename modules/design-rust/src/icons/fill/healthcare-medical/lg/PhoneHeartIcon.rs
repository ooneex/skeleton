use dioxus::prelude::*;

#[derive(Props, Clone, PartialEq)]
pub struct PhoneHeartIconProps {
    #[props(extends = svg, extends = GlobalAttributes)]
    pub attributes: Vec<Attribute>,
}

#[component]
pub fn PhoneHeartIcon(props: PhoneHeartIconProps) -> Element {
    rsx! {
        svg {
            height: "16",
            width: "16",
            view_box: "0 0 48 48",
            xmlns: "http://www.w3.org/2000/svg",
            fill: "none",
            ..props.attributes,
            path {
                fill_rule: "evenodd",
                clip_rule: "evenodd",
                d: "M16 43L32 43C33.6569 43 35 41.6569 35 40L35 8.00002C35 6.34317 33.6569 5.00002 32 5.00002L16 5.00002C14.3432 5.00002 13 6.34316 13 8.00002L13 40C13 41.6569 14.3431 43 16 43ZM32 46L16 46C12.6863 46 10 43.3137 10 40L10 8.00002C10 4.68631 12.6863 2.00002 16 2.00002L32 2.00002C35.3137 2.00002 38 4.68631 38 8.00002L38 40C38 43.3137 35.3137 46 32 46Z",
                fill: "currentColor",
            }
            path {
                fill_rule: "evenodd",
                clip_rule: "evenodd",
                d: "M20 8.5C20 7.67157 20.6716 7 21.5 7L26.5 7C27.3284 7 28 7.67157 28 8.5C28 9.32843 27.3284 10 26.5 10L21.5 10C20.6716 10 20 9.32843 20 8.5Z",
                fill: "currentColor",
            }
            path {
                d: "M24 35C25.9962 34.0588 33 28.8428 33 24.0616C33 21.2664 30.8292 19 28.1544 19C26.3904 19 25.0728 20.1558 24 21.4527C22.929 20.1539 21.6096 19 19.8456 19C17.169 19 15 21.2664 15 24.0616C15 28.8428 22.0038 34.0588 24 35Z",
                fill: "currentColor",
                "data-color": "color-2",
            }
        }
    }
}
