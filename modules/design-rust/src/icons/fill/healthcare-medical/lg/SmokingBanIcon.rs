use dioxus::prelude::*;

#[derive(Props, Clone, PartialEq)]
pub struct SmokingBanIconProps {
    #[props(extends = svg, extends = GlobalAttributes)]
    pub attributes: Vec<Attribute>,
}

#[component]
pub fn SmokingBanIcon(props: SmokingBanIconProps) -> Element {
    rsx! {
        svg {
            height: "16",
            width: "16",
            view_box: "0 0 48 48",
            xmlns: "http://www.w3.org/2000/svg",
            fill: "none",
            ..props.attributes,
            path {
                d: "M41 18C41 13.3056 37.1944 9.5 32.5 9.5H32V6.5L32.5 6.5C38.8513 6.5 44 11.6487 44 18V23H41V18Z",
                fill: "currentColor",
                "data-color": "color-2",
            }
            path {
                d: "M36 21C36 19.067 34.433 17.5 32.5 17.5C28.9101 17.5 26 14.5899 26 11L26 6.5H29V11C29 12.933 30.567 14.5 32.5 14.5C36.0899 14.5 39 17.4101 39 21V23H36V21Z",
                fill: "currentColor",
                "data-color": "color-2",
            }
            path {
                d: "M33 34H22.1348L16.8086 27H33V34Z",
                fill: "currentColor",
            }
            path {
                d: "M16.4814 34H4V27H11.1553L16.4814 34Z",
                fill: "currentColor",
            }
            path {
                d: "M30.7917 42.8997L28.404 44.7161L8.17157 18.1234L10.5583 16.3069L30.7917 42.8997Z",
                fill: "currentColor",
                "data-color": "color-2",
            }
            path {
                d: "M41 27C42.6569 27 44 28.3431 44 30V31C44 32.6569 42.6569 34 41 34H36V27H41Z",
                fill: "currentColor",
            }
        }
    }
}
